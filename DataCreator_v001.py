...
# record clicks
    ...
    point_ids.append(Particle(x = x, y = y))
    # update the gui
    update_gui_text()
...
def file_save():
    ...
    global xe, ye, e_cat, x_var_entry, y_var_entry
    data = pd.DataFrame()
    x = x_var_entry.get()
    y = y_var_entry.get()
    category = x2_var_entry.get()
    data[x] = np.asarray(clicks)[:,0] / canvas.winfo_width()  # This changes the data so that (0,0) is SW, so it works correctly when plotting
    data[y] = np.asarray(clicks)[:,1] / canvas.winfo_height() # This changes the data so that (0,0) is SW, so it works correctly when plotting
...
def update_gui_text():
    ...
    # This is all to change the text that appears in the gui
